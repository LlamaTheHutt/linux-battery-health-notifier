...
if [ -f "$BATTERY_PATH" ]; then
    ...
    if [ "$STATUS" = "Charging" ]; then
        if [ "$battery_percentage" -gt 80 ]; then
            XDG_RUNTIME_DIR=/run/user/$(id -u) notify-send -u critical "Battery Warning" "Battery is at ${battery_percentage}%. Please unplug your charger."
        else
            echo "Still Charging..."
        fi
    elif [ "$STATUS" = "Discharging" ]; then
        if [ "$battery_percentage" -lt 20 ]; then
            XDG_RUNTIME_DIR=/run/user/$(id -u) notify-send -u critical "Battery Warning" "Battery is at ${battery_percentage}%. Please plug in your charger."
        else
